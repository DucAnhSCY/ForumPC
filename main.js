...
// QUAN TRỌNG: Token này chỉ nên được sử dụng trong môi trường phát triển
// Trong môi trường sản xuất, đây nên là biến môi trường server-side
const CF_API_TOKEN = 'gmDtEhZOhrYf8ilTdpVwyudvEhuZUa5p_OmEDcwP';
...
async function uploadImageToCloudflare(file) {
    try {
        ...
        formData.append('requireSignedURLs', 'false'); // Set to true if you want signed URLs

        const response = await fetch(CF_API_ENDPOINT, {
            method: 'POST',
            headers: {
                // For development only - should be replaced with secure server-side handling in production
...
/**
 * Handler for CKEditor image uploads to Cloudflare
 */
function setupCloudflareImageUpload() {
    ...
    // Create an API endpoint for CKEditor
    if (typeof window.CKEDITOR !== 'undefined') {
        // Set up XHR override for CKEditor's upload URL
        window.CKEDITOR.on('dialogDefinition', function(ev) {
            ...
            if (dialogName === 'image') {
                ...
                if (uploadTab) {
                    // Override the onShow function to handle custom upload
                    const originalOnShow = dialogDefinition.onShow || function() {};
                    dialogDefinition.onShow = function() {
                        originalOnShow.call(this);
                        
                        // Get the upload input
                        const uploadButton = this.getContentElement('Upload', 'uploadButton');
                        if (uploadButton) {
        ...
        fileInput.addEventListener('change', async function() {
            if (this.files && this.files[0]) {
                try {
                    ...
                    showCustomAlert('Uploading', 'Uploading image to Cloudflare...', 'info');
                    
                    // Upload to Cloudflare
                    const imageUrl = await uploadImageToCloudflare(this.files[0]);
                    
                    if (imageUrl) {
                        // Insert the image into the active editor
                        const editor = window.CKEDITOR.currentInstance;
                        if (editor) {
                            editor.insertHtml(`<img src="${imageUrl}" alt="Uploaded Image" />`);
                            showCustomAlert('Success', 'Image uploaded successfully!', 'success');
                        }
                    }
...
// Initialize Cloudflare image upload when page loads
document.addEventListener('DOMContentLoaded', function() {
    setupCloudflareImageUpload();
